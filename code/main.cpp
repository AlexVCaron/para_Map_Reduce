...
int main(int argc, char* argv[])
{
    ...
    if (exec_parallele) {
        for (i = 0; i < nb_threads - 1; ++i)
            ...
            }, f_d.path, f_d.splitFiles(i * file_share, ((i + 1) * file_share) - 1), file_reader(w_i), timer(), &v_time[i], &nb_w_treated));
    }
    ...
    if (remaining_share != 0) {
        m_p.resize(1);
        unsigned nb_words_read = 0;
        vector<string> files = f_d.splitFiles(f_d.nb_files - remaining_share - 1, f_d.nb_files - 1);
        timer t;
        auto t_start = t.now();
        nb_w_treated.registerOperation();
